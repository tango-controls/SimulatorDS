...
try:
  ...
  import fandango
  #class ExecutorClass(fandango.SingletonWorker):
    #def submit(self,f,*args,**kwargs):
      #print(f,args,kwargs)
      #k = ((lambda y,x=f:x(*y)),args)
      #self.put(k)
      #while not self.done(): self.stopEvent.wait(.01)
      #return self.get(k)
  class ExecutorClass(fandango.WorkerProcess):
    def submit(self,f,*args,**kwargs):
      return self.command(command=f,args=args)
...
class Tango_dbapi2(object):
    ...
    def purge_att_property(self, table, field, obj, attr, name):
        ...
        cursor.execute(\
            ...
            'attribute = ? ORDER BY date COLLATE NOCASE', (table, field, obj, name, attr))
    ...
    def purge_property(self, table, field, obj, name):
        cursor = self.cursor
        cursor.execute(\
            'SELECT DISTINCT id FROM ? WHERE ? = ? AND name = ? COLLATE NOCASE ' \
            'ORDER BY date', (table, field, obj, name))
        rows = cursor.fetchall()
        to_del = len(rows) - self.history_depth
        if to_del > 0:
            for row in rows[:to_del]:
                cursor.execute('DELETE FROM ? WHERE id=? COLLATE NOCASE', (table, row[0]))

    @use_cursor
    def get_device_host(self, name):
        cursor = self.cursor
        name = replace_wildcard(name)
        cursor.execute('SELECT host FROM device WHERE name LIKE ? COLLATE NOCASE', (name,))
        row = cursor.fetchone()
        if row is None:
    ...
    def add_device(self, server_name, dev_info, klass_name, alias=None):
        ...
        cursor.execute('DELETE FROM device WHERE name LIKE ? COLLATE NOCASE', (dev_name,))
    ...
    def delete_attribute_alias(self, alias):
        ...
        self.cursor.execute('DELETE FROM attribute_alias WHERE alias=? COLLATE NOCASE', (alias,))

    @use_cursor
    def delete_class_attribute(self, klass_name, attr_name):
        self.cursor.execute(\
            'DELETE FROM property_attribute_class WHERE class LIKE ? AND ' \
            'attribute LIKE ? COLLATE NOCASE', (klass_name, attr_name))
    ...
    def delete_class_attribute_property(self, klass_name, attr_name, prop_name):
        ...
        cursor.execute(\
            ...
            'AND attribute = ? AND name = ? COLLATE NOCASE', (klass_name, attr_name, prop_name))
        if cursor.fetchone()[0] > 0:
            # then delete property from the property_attribute_class table
            cursor.execute(\
                'DELETE FROM property_attribute_class WHERE class = ? AND ' \
                'attribute = ? and name = ? COLLATE NOCASE', (klass_name, attr_name, prop_name))
    ...
    def delete_class_property(self, klass_name, prop_name):
        ...
        cursor.execute(\
            ...
            'name LIKE ? COLLATE NOCASE', (klass_name, prop_name))
        for row in cursor.fetchall():
            # delete the tuple (device,name,count) from the property table
            name = row[0]
            cursor.execute(\
                'DELETE FROM property_class WHERE class=? AND name=? COLLATE NOCASE',
                (klass_name, name))
            # Mark this property as deleted
    ...
    def delete_device(self, dev_name):
        ...
        cursor.execute('DELETE FROM property_device WHERE device LIKE ? COLLATE NOCASE', (dev_name,))

        # delete device from the property_attribute_device table
        cursor.execute('DELETE FROM property_attribute_device WHERE device LIKE ? COLLATE NOCASE', (dev_name,))

    @use_cursor
    def delete_device_alias(self, dev_alias):
        self._info("delete_device_alias(dev_alias=%s)", dev_alias)
        self.cursor.execute('UPDATE device SET alias=NULL WHERE alias=? COLLATE NOCASE', (dev_alias,))
    ...
    def delete_device_attribute(self, dev_name, attr_name):
        ...
        self.cursor.execute(\
            ...
            'attribute LIKE ? COLLATE NOCASE', (dev_name, attr_name))
    ...
    def delete_device_attribute_property(self, dev_name, attr_name, prop_name):
        ...
        cursor.execute(\
            ...
            'AND attribute = ? AND name = ? COLLATE NOCASE', (dev_name, attr_name, prop_name))
        if cursor.fetchone()[0] > 0:
            # delete property from the property_attribute_device table
            cursor.execute(\
                'DELETE FROM property_attribute_device WHERE device = ? AND '
                'attribute = ? AND name = ? COLLATE NOCASE', (dev_name, attr_name, prop_name))
            # Mark this property as deleted
    ...
    def delete_device_property(self, dev_name, prop_name):
        ...
        cursor.execute(\
            ...
            'name LIKE ? COLLATE NOCASE', (dev_name, prop_name))
        for row in cursor.fetchall():
            # delete the tuple (device,name,count) from the property table
            cursor.execute(\
                'DELETE FROM property_device WHERE device=? AND name LIKE ? COLLATE NOCASE',
                (dev_name, prop_name))
            # Mark this property as deleted
    ...
    def delete_property(self, obj_name, prop_name):
        ...
        cursor.execute(\
            ...
            'name LIKE ? COLLATE NOCASE', (obj_name, prop_name))
        for row in cursor.fetchall():
    ...
    def delete_server(self, server_instance):
        ...
        cursor.execute('DELETE FROM device WHERE server LIKE ? COLLATE NOCASE', (server_instance,))
    ...
    def delete_server_info(self, server_instance):
        self.cursor.execute('DELETE FROM server WHERE name=? COLLATE NOCASE', (server_instance,))
    ...
    def export_device(self, dev_name, IOR, host, pid, version):
        ...
        if self.fire_to_starter:
            print('export_device(): fire to starter')
            if dev_name.lower().startswith("dserver/database"): #"dserver/":
        ...
        print('export_device(): find in the database')
        cursor.execute('SELECT server FROM device WHERE name LIKE ? COLLATE NOCASE', (dev_name,))
        row = cursor.fetchone()
        if row is None:
        ...
        print('export_device(): update the new value for this tuple')
        cursor.execute(\
            'UPDATE device SET exported=1, ior=?, host=?, pid=?, version=?, ' \
            'started=datetime("now") WHERE name LIKE ? COLLATE NOCASE',
            (IOR, host, pid, version, dev_name))

        print('export_device(): update host name in server table')
        cursor.execute('UPDATE server SET host=? WHERE name LIKE ? COLLATE NOCASE', (host, server))

        if do_fire:
            print('export_device(): do fire')
            hosts = []
            hosts.append(host)
    ...
    def get_alias_device(self, dev_alias):
        cursor = self.cursor
        cursor.execute('SELECT name FROM device WHERE alias LIKE ? COLLATE NOCASE',
                       (dev_alias,))
        row = cursor.fetchone()
        if row is None:
            th_exc(DB_DeviceNotDefined,
    ...
    def get_attribute_alias(self, attr_alias):
        cursor = self.cursor
        cursor.execute('SELECT name from attribute_alias WHERE alias LIKE ? COLLATE NOCASE',
                       (attr_alias,))
        row = cursor.fetchone()
        if row is None:
            th_exc(DB_SQLError,
    ...
    def get_attribute_alias_list(self, attr_alias):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT alias FROM attribute_alias WHERE alias LIKE ? ORDER BY attribute COLLATE NOCASE',
                       (attr_alias,))
    ...
    def get_class_attribute_list(self, class_name, wildcard):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT attribute FROM property_attribute_class WHERE class=? and attribute like ? COLLATE NOCASE',
                       (class_name, wildcard))
        return [ row[0] for row in cursor.fetchall() ]
    ...
    def get_class_attribute_property(self, class_name, attributes):
        cursor = self.cursor
        stmt = 'SELECT name,value FROM property_attribute_class WHERE class=? AND attribute LIKE ? COLLATE NOCASE'
    ...
    def get_class_attribute_property2(self, class_name, attributes):
        cursor = self.cursor
        stmt = 'SELECT name,value FROM property_attribute_class WHERE class=? AND attribute LIKE ? ORDER BY name,count COLLATE NOCASE'
        result = [class_name, str(len(attributes))]
        for attribute in attributes:
            cursor.execute(stmt, (class_name, attribute))
            rows = cursor.fetchall()
    ...
    def get_class_attribute_property_hist(self, class_name, attribute, prop_name):
        cursor = self.cursor
        stmt = 'SELECT  DISTINCT id FROM property_attribute_class_hist WHERE class=? AND attribute LIKE ? AND name LIKE ? ORDER by date ASC COLLATE NOCASE'
        ...
        for row in cursor.fetchall():
            ...
            stmt = 'SELECT DATE_FORMAT(date,\'%Y-%m-%d %H:%i:%s\'),value,attribute,name,count FROM property_attribute_class_hist WHERE id =? AND class =? COLLATE NOCASE'
    ...
    def get_class_for_device(self, dev_name):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT class FROM device WHERE name=? COLLATE NOCASE', (dev_name,))
        row = cursor.fetchone()
        if row is None:
    ...
    def get_class_list(self, server):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT class FROM device WHERE class LIKE ? ORDER BY class COLLATE NOCASE', (server,))
        return [ row[0] for row in cursor.fetchall() ]


    @use_cursor
    def get_class_property(self, class_name, properties):
        print(class_name,properties)
        result = []
        cursor = self.cursor
        stmt = 'SELECT count,value FROM property_class WHERE class=? AND name LIKE ? ORDER BY count COLLATE NOCASE'
    ...
    def get_class_property_hist(self, class_name, prop_name):
        cursor = self.cursor
        stmt = 'SELECT  DISTINCT id FROM property_class_hist WHERE class=? AND AND name LIKE ? ORDER by date ASC COLLATE NOCASE'
        ...
        for row in cursor.fetchall():
            ...
            stmt = 'SELECT DATE_FORMAT(date,\'%Y-%m-%d %H:%i:%s\'),value,name,count FROM property_class_hist WHERE id =? AND class =? COLLATE NOCASE'
    ...
    def get_class_property_list(self, class_name):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT name FROM property_class WHERE class LIKE ? order by NAME COLLATE NOCASE',
                       (class_name,))
        return [ row[0] for row in cursor.fetchall() ]
    ...
    def get_device_alias(self, dev_name):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT alias FROM device WHERE name LIKE ? COLLATE NOCASE',
                       (dev_name,))
        row = cursor.fetchone()
        if row is None:
            th_exc(DB_DeviceNotDefined,
    ...
    def get_device_alias_list(self, alias):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT alias FROM device WHERE alias LIKE ? ORDER BY alias COLLATE NOCASE',
                       (alias,))
        return [ row[0] for row in cursor.fetchall() ]
    ...
    def get_device_attribute_list(self, dev_name, attribute):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT  attribute FROM property_attribute_device WHERE device=?  AND attribute LIKE ? ORDER BY attribute COLLATE NOCASE',
                       (dev_name, attribute,))
        return [ row[0] for row in cursor.fetchall() ]
    ...
    def get_device_attribute_property(self, dev_name, attributes):
        cursor = self.cursor
        stmt = 'SELECT name,value FROM property_attribute_device WHERE device=? AND attribute LIKE ? COLLATE NOCASE'
        ...
        for attribute in attributes:
            ...
            for row in rows:
                result.append(row[0])
                result.append(row[1])
            if not rows: result.append(' ') #By convention it shouldn't be like this; but this is what the C++ DatabaseDS is sending to clients
        return result  

    @use_cursor
    def get_device_attribute_property2(self, dev_name, attributes):
        cursor = self.cursor
        stmt = 'SELECT name,value FROM property_attribute_device WHERE device=? AND attribute LIKE ? ORDER BY name,count COLLATE NOCASE' 
        result = [dev_name, str(len(attributes))]
        for attribute in attributes:
            cursor.execute(stmt, (dev_name, attribute))
            rows = cursor.fetchall()
    ...
    def get_device_attribute_property_hist(self, dev_name, attribute, prop_name):
        cursor = self.cursor
        stmt = 'SELECT  DISTINCT id FROM property_attribute_device_hist WHERE device=? AND attribute LIKE ? AND name LIKE ? ORDER by date ASC COLLATE NOCASE'
        ...
        for row in cursor.fetchall():
            ...
            stmt = 'SELECT DATE_FORMAT(date,\'%Y-%m-%d %H:%i:%s\'),value,attribute,name,count FROM property_attribute_device_hist WHERE id =? AND device =? ORDER BY count ASC COLLATE NOCASE'
    ...
    def get_device_class_list(self, server_name):
        cursor = self.cursor
        result = []
        cursor.execute('SELECT name,class FROM device WHERE server =?  ORDER BY name COLLATE NOCASE',
                       (server_name,))
        for row in cursor.fetchall():
            result.append(row[0])
            result.append(row[1])
    ...
    def get_device_domain_list(self, wildcard):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT domain FROM device WHERE name LIKE ? OR alias LIKE ? ORDER BY domain COLLATE NOCASE',
                       (wildcard,wildcard))
        return [ row[0] for row in cursor.fetchall() ]
    ...
    def get_device_exported_list(self, wildcard):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT name FROM device WHERE (name LIKE ? OR alias LIKE ?) AND exported=1 ORDER BY name COLLATE NOCASE',
                       (wildcard,wildcard))
        return [ row[0] for row in cursor.fetchall() ]   
    ...
    def get_device_family_list(self, wildcard):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT family FROM device WHERE name LIKE ? OR alias LIKE ? ORDER BY family COLLATE NOCASE',
                       (wildcard,wildcard))
        return [ row[0] for row in cursor.fetchall() ]   
    ...
    def get_device_info(self, dev_name):
        cursor = self.cursor
        cursor.execute('SELECT exported,ior,version,pid,server,host,started,stopped,class FROM device WHERE name =?  or alias =? COLLATE NOCASE',
                       (dev_name,dev_name))
        result_long = []
        result_str = []
        for row in cursor.fetchall():
            ...
            row = [(r if r not in (None,'nada') else '') for r in row ]
            result_str.append(dev_name)
            result_str.append(str(row[1])) #IOR
            result_str.append(str(row[2]) or "0") #Version
            result_str.append(str(row[4])) #Server
            result_str.append(str(row[5])) #Hostname
           
            for i in range(0,2):
                #cursor.execute('SELECT DATE_FORMAT(?,\'%D-%M-%Y at %H:%i:%s\')', row[6 + i])
                #tmp_date = cursor.fetchone()
                #if tmp_date == None:
                    #result_str.append("?")
                #else:               
                    #result_str.append(str(tmp_date))
                result_str.append(str(row[6+i] or "")) #Started/Stop

            for i in (0,3):
                if row[i] != None:
                    result_long.append(int(row[i] or 0))
    ...
    def get_device_list(self,server_name, class_name ):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT name FROM device WHERE server LIKE ? AND class LIKE ? ORDER BY name COLLATE NOCASE',
    ...
    def get_device_wide_list(self, wildcard):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT name FROM device WHERE name LIKE ? ORDER BY name COLLATE NOCASE',
                       (wildcard,))
        return [ row[0] for row in cursor.fetchall() ]
    ...
    def get_device_member_list(self, wildcard):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT  member FROM device WHERE name LIKE ? ORDER BY member COLLATE NOCASE',
                       (wildcard,))
        return [ row[0] for row in cursor.fetchall() ]    
    ...
    def get_device_property(self, dev_name, properties):
        cursor = self.cursor
        stmt = 'SELECT count,value,name FROM property_device WHERE device = ? AND name LIKE ?  ORDER BY count COLLATE NOCASE'
        result = []
        ...
        for prop in properties:
            ...
            if rows:
                for row in rows:
                    result.append(row[1])
            else: result.append(' ') #By convention it shouldn't be like this; but this is what the C++ DatabaseDS is sending to clients
        return result    
      
    @use_cursor
    def get_device_property_list(self, device_name,prop_filter):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT name FROM property_device WHERE device = ? and name like ? order by NAME  COLLATE NOCASE',
    ...
    def get_device_property_hist(self, device_name, prop_name):
        cursor = self.cursor
        stmt = 'SELECT  DISTINCT id FROM property_device_hist WHERE device=? AND name LIKE ? ORDER by date ASC COLLATE NOCASE'
        ...
        stmt = 'SELECT DATE_FORMAT(date,\'%Y-%m-%d %H:%i:%s\'),value,name,count FROM property_device_hist WHERE id =? AND device =? ORDER BY count ASC COLLATE NOCASE'
    ...
    def get_device_server_class_list(self, server_name):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT  class FROM device WHERE server LIKE ? ORDER BY class COLLATE NOCASE',
                       (sever_name,))
        return [ row[0] for row in cursor.fetchall() ]
    ...
    def get_exported_device_list_for_class(self, class_name):
        cursor = self.cursor
        cursor.execute('SELECT  DISTINCT name FROM device WHERE class LIKE ? AND exported=1 ORDER BY name COLLATE NOCASE',
                       (class_name,))
        return [ row[0] for row in cursor.fetchall() ]   
    ...
    def get_object_list(self, name):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT object FROM property WHERE object LIKE ?  ORDER BY object COLLATE NOCASE',
                       (name,))
        return [ row[0] for row in cursor.fetchall() ]
    ...
    def get_property(self, object_name, properties):
        ...
        result.append(str(len(properties))) 
        stmt = 'SELECT count,value,name FROM property WHERE object LIKE ?  AND name LIKE ? ORDER BY count COLLATE NOCASE'
        for prop_name in properties:
            result.append(prop_name)
            prop_name = replace_wildcard(prop_name)
    ...
    def get_property_hist(self, object_name, prop_name):
        ...
        stmt = 'SELECT  DISTINCT id FROM property_hist WHERE object=? AND name LIKE ? ORDER by date COLLATE NOCASE'        
        prop_name = replace_wildcard(prop_name)        
        cursor.execute(stmt, (object_name, prop_name))

        stmt = 'SELECT DATE_FORMAT(date,\'%Y-%m-%d %H:%i:%s\'),value,name,count FROM property_hist WHERE id =? AND object =? COLLATE NOCASE'
        for row in cursor.fetchall():
            idr = row[0]
    ...
    def get_property_list(self, object_name, wildcard):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT name FROM property WHERE object LIKE ? AND name LIKE ? ORDER BY name COLLATE NOCASE',
    ...
    def get_server_info(self, server_name):
        cursor = self.cursor
        cursor.execute('SELECT host,mode,level FROM server WHERE name =? COLLATE NOCASE',
                       (server_name,))
        result = []
    ...
    def get_server_list(self, wildcard):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT server FROM device WHERE server LIKE ? ORDER BY server COLLATE NOCASE',
                       (wildcard,))
    ...
    def get_server_class_list(self, wildcard):
        cursor = self.cursor
        cursor.execute('SELECT DISTINCT class FROM device WHERE server LIKE ? ORDER BY server COLLATE NOCASE',
                       (wildcard,))
        return [ row[0] for row in cursor.fetchall() ]
    ...
    def import_device(self, dev_name):
        ...
        cursor.execute('SELECT exported,ior,version,pid,server,host,class FROM device WHERE name =? COLLATE NOCASE',
                       (dev_name,))
        rows = cursor.fetchall()
        if len(rows) == 0:
            cursor.execute('SELECT exported,ior,version,pid,server,host,class FROM device WHERE alias =? COLLATE NOCASE',
                           (dev_name,))
            rows = cursor.fetchall()
            if len(rows) == 0:
                th_exc(DB_DeviceNotDefined,
        ...
        for row in rows:
            print(row)
            result_str.append(dev_name)
            result_str.append(row[1] if row[1]!=None else "") #Corba IOR
            result_str.append(row[2] if row[0]!='nada' else '0') #Device Version
            result_str.append(row[4]) #Device server process name
            result_str.append(row[5]) #Host name
            result_str.append(row[6]) #Class name
            result_long.append(row[0] if row[0]!='nada' else 0) #exported flag
            result_long.append(row[3] if row[0]!='nada' else 0) #PID
        result = (result_long, result_str)
        return result    
    ...
    def import_event(self, event_name):
        ...
        result_long = []
        result_str = []
        cursor.execute('"SELECT exported,ior,version,pid,host FROM event WHERE name =? COLLATE NOCASE',
                       (event_name,))
        rows = cursor.fetchall()
        if len(rows) == 0:
            th_exc(DB_DeviceNotDefined,
    ...
    def put_class_property(self, class_name, nb_properties, attr_prop_list):
        ...
        for i in range(0,nb_properties):
            ...
            for j in range(k+2,k+n_rows+2,1):
                ...
                tmp_count = tmp_count+1
                # then insert the new value for this tuple
                cursor.execute('INSERT INTO property_class ' \
                    '(class, name, count, value, updated, accessed)' \
                    ' VALUES(?, ?, ?, ?, "nada", "nada")',
                    #SET class=? ,name=?,count=?,value=?,updated=NULL,accessed=NULL', 
    ...
    def put_device_attribute_property(self, device_name, nb_attributes, attr_prop_list):
        ...
        for i in range(0,nb_attributes):
            ...
            for j in range(k+2,k+nb_properties*2+2,2):
                ...
                # then insert the new value for this tuple
                cursor.execute('INSERT INTO property_attribute_device ' \
                    '(device, attribute, name, count, value, updated, accessed) '\
                    'VALUES (?, ?, ?, \'1\', ?, NULL, NULL)', 
                    #'SET device=? ,attribute=?,name=?,count=\'1\',value=?,updated=NULL,accessed=NULL',
                    (device_name, tmp_attribute, tmp_name, tmp_value))
                # then insert the new value into the history table
                #hist_id = self.get_id("device_attribute", cursor=cursor)
                #cursor.execute('INSERT INTO property_attribute_device_hist ' \
                    #'(device, attribute, name, id, count, value) ' \
                    #'VALUES(?, ?, ?, ?, 1, ?)',
                    ##'SET device=?,attribute=?,name=?,id=?,count=\'1\',value=?', 
                    #(device_name, tmp_attribute, tmp_name,hist_id,tmp_value))
                ...
                for jj in range(0,nb_properties,1):
                    ...
                    for l in range(j+1,j+n_rows+1,1):
                        ...
                        cursor.execute('INSERT INTO property_attribute_device ' \
                            '(device, attribute, name, count, value, updated, accessed) '\
                            'VALUES (?, ?, ?, ?, ?, "nada", "nada")', 
                            (device_name, tmp_attribute, tmp_name, str(tmp_count), tmp_value))                        
                        # then insert the new value into the history table
                        #hist_id = self.get_id("device_attribute", cursor=cursor)
    ...
    def put_device_property(self, device_name, nb_properties, attr_prop_list):
        print('put_device_property(%s,%s,%s)'%(device_name,nb_properties,attr_prop_list))
        try:
          cursor = self.cursor
          k = 0
          #hist_id = self.get_id("device", cursor=cursor)
          for i in range(0,nb_properties):
              ...
              for j in range(k+2,k+n_rows+2,1):
                  ...
                  cursor.execute('INSERT INTO property_device (device,name,count,value,updated,accessed) VALUES (?,?,?,?,?,?)', (device_name, tmp_name, str(tmp_count), tmp_value,time.time(),time.time()))
              ...
              #self.purge_att_property("property_device_hist", "device",device_name, tmp_name, cursor=cursor)
              k = k + n_rows + 2
        except:
          import traceback
          traceback.print_exc()
    ...
    def put_property(self, object_name, nb_properties, attr_prop_list):
        ...
        for i in range(0,nb_properties):
            ...
            #self.purge_att_property("property_hist", "object",object_name, tmp_name, cursor=cursor)
            k = k + n_rows + 2
